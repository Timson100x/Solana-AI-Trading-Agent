...
export class NLCommandHandler {
  ...
  initializeCommands() {
    return {
      ...
      // DCA Bot
      dcabot: {
        keywords: ["dca", "dollar cost", "durchschnitt", "regelmäßig"],
        template: "dca-bot",
   ...
   */
  async parse(input) {
    logger.info(`🧠 Parsing NL command: "${input}"`);
    ...
    logger.success(`✅ Parsed: ${detectedCommand.description}`);
...
`;
    ...
    try {
      ...
      logger.warn("⚠️ Parameter extraction failed:", error.message);
      return {};
    }
  }
  ...
  async generateCode(spec) {
    logger.info(`🔨 Generating code for: ${spec.type}`);
...
class GridTradingBot {
  ...
  async initialize() {
    ...
    logger.success(\`✅ Grid initialized: \${levels} levels from \${minPrice} to \${maxPrice}\`);
  }
  ...
  async start() {
    ...
    while (true) {
      try {
        ...
        logger.error("❌ Grid cycle error:", error);
  ...
  async placeBuyOrder(level) {
    try {
      logger.info(\`📉 Placing buy order at \${level.price}\`);
      ...
      logger.success(\`✅ Buy order placed: \${result.signature.slice(0, 8)}...\`);
    } catch (error) {
  ...
  async placeSellOrder(level) {
    try {
      logger.info(\`📈 Placing sell order at \${level.price}\`);
      ...
      logger.success(\`✅ Sell order placed: \${result.signature.slice(0, 8)}...\`);
    } catch (error) {
...
class DCABot {
  ...
  async start() {
    ...
    while (true) {
      try {
        ...
        logger.error("❌ DCA cycle error:", error);
        await this.sleep(60000);
      }
    }
  }
  ...
  async executePurchase() {
    ...
    logger.success(\`✅ Purchased: \${result.signature.slice(0, 8)}...\`);
...
`;
  ...
  /**
   * Sniper Bot Template (simplified)
   */
  sniperBotTemplate() {
    return `// Sniper Bot Template - Simplified version`;
  }
